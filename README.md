...
「立即掃 ETtoday」呼叫 `POST /api/ettoday_scan`。「啟動定時掃描」會在網頁開著時定時掃描；「只顯示新新聞」預設開啟，瀏覽器通知也只通知新新聞。
...
- 右側：建議動作、RAP 分數、初步可唱分、建議曲風、風險等級、已抓內文字數、抓取狀態、新新聞 / 已看過、原文連結、產製作包按鈕。

建議動作規則：

- `rap_score >= 4.3`、`risk_level = low`、`article_text_length >= 800`：可做。
- `risk_level = medium` 或 RAP 分數 3.8 到 4.29：觀望。
- `risk_level = high` 或 `article_text_length < 300`：不建議。
...
按「掃描這批新聞」會呼叫 `POST /api/daily_topics`，結果會出現在「2｜候選題清單」。

### 4｜單篇 RAP 製作

此區只負責原本單篇新聞產製。已經決定要做某一則時，貼上一則新聞網址或完整新聞內文，按「生成這則 RAP」會呼叫原本 `POST /api/analyze`，後續仍可編輯歌詞、生成音樂、生成影片與下載素材。
